...
int main(void)
{
	...
	gpio_init(GPIOA, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_5);//PA5配置成输出
	//zhiwen add 23 - 02 - 26
	gpio_init(GPIOA, GPIO_MODE_OUT_PP, GPIO_OSPEED_50MHZ, GPIO_PIN_6);//PA6配置成输出
	while(1)
	{
		delay_1ms(1000);//等待1000ms
		gpio_bit_set(GPIOA, GPIO_PIN_4);//输出高
		gpio_bit_set(GPIOA, GPIO_PIN_5);//输出高
		gpio_bit_set(GPIOA, GPIO_PIN_6);//输出高
		gpio_bit_set(GPIOB, GPIO_PIN_4);//输出高
		delay_1ms(1000);
		gpio_bit_reset(GPIOA, GPIO_PIN_4);//输出低
		gpio_bit_reset(GPIOA, GPIO_PIN_5);//输出低 
		gpio_bit_reset(GPIOA, GPIO_PIN_6);//输出低
